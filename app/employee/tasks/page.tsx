...
export default function EmployeeTasksPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Load tasks assigned to employee (task receiver role)
    ...
    return (
      <div className="max-w-[1200px] mx-auto p-6">
        <div className="text-center py-16 text-tx3">
          ...
          <div className="text-sm">My tasks loading...</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-[1200px] mx-auto p-6">
      {/* Header */}
      <div className="mb-6">
        <div className="text-xs text-tx3 mb-1">Employee / Uureg</div>
        <h1 className="text-xl font-bold">Uureg avsan daalgavaruud</h1>
        <p className="text-sm text-tx2 mt-0.5">Tand ogson daalgavaruud</p>
      </div>

      {/* Export Button */}
      <div className="mb-6">
        <button className="px-4 py-2 bg-success text-white rounded-lg text-sm font-medium hover:bg-success-light transition-colors">
          📊 Export My Tasks
        </button>
      </div>
      ...
      <div className="bg-surface border border-border rounded-lg p-4 mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <input
              type="text"
              placeholder="Minii uureg hiij khariulch avna..."
              value={search}
        ...
        {!filteredTasks.length ? (
          ...
          filteredTasks.map((task) => (
            <Link
              key={task.id}
              href={`/tasks/${task.id}`}
              className="block bg-surface border border-border rounded-lg p-4 hover:border-success/50 transition-colors"
            >
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total', value: tasks.length, color: 'text-success-light' },
