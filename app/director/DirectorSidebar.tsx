...
export function DirectorSidebar({
  ...
  const navItems: SidebarItem[] = [
    { label: 'Хяналтын самбар', href: '/director', icon: '📊' },
  ...
  const bottomItems: SidebarItem[] = [
    { label: 'Тохиргоо', href: '/director/settings', icon: '⚙️' },
  ]
    ...
    <aside className="w-[220px] flex-shrink-0 bg-surface border-r border-border flex flex-col h-full">
      ...
      <div className="px-4 py-4 border-b border-border">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded bg-primary/20 flex items-center justify-center text-primary-light text-xs font-bold">ГШ</div>
          <div>
      ...
      <nav className="flex-1 py-3 overflow-y-auto">
        <div className="px-3 mb-1">
          <span className="text-[10px] font-bold text-tx3 uppercase tracking-widest px-2">Захирал цэс</span>
        </div>
        {navItems.map(item => {
          const active = path === item.href || path.startsWith(item.href + '/')
          const count = item.href === '/director/tasks' ? overdueCount : 0
          return (
            <Link key={item.href} href={item.href}
              className={cn(
                'flex items-center gap-2.5 mx-2 px-3 py-2 rounded text-sm transition-all duration-100 border border-transparent',
                active
                  ? 'bg-primary/10 text-tx border-primary/20 border-l-2 border-l-primary-light'
