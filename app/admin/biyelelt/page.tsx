...
export default async function BiyeleltPage() {
  ...
  const isReadOnly = profile?.role === 'admin' // Admins can only view
  const orgId = profile?.org_id ?? ''

  const { data: tasks } = await supabase.from('tasks_full').select('*').eq('org_id', orgId).order('deadline')

  const { data: staffStats } = await supabase.from('staff_stats').select('*').order('completion_pct', { ascending: false })
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-tx2 mt-0.5">
            {isReadOnly ? 'Бүх биелэлтийг харах (Зөвхөн харах)' : canManage ? 'Бүх ажилтны биелэлтийг харах' : 'Танд хариуцсан үүрэг биелэлт'}
          </p>
          {isReadOnly && (
            <div className="mt-2 p-2 bg-amber-500/10 border border-amber-500/30 rounded-lg text-xs text-amber-400">
              Админ хэрэглэгчид зөвхөн харах боломжтой. Өөрчлөлт оруулах боломжгүй.
            </div>
          )}
        </div>
        {isReadOnly ? (
          <div className="text-xs text-tx3 bg-gray-500/10 px-3 py-2 rounded-lg">
            👁️ Харах горим
          </div>
        ) : canManage ? (
