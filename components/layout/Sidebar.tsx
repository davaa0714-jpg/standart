...
// Админ sidebar - Бүх бүрэн эрхтэй
    ...
    <aside className="w-[220px] flex-shrink-0 bg-surface border-r border-border flex flex-col h-full">
      ...
      <nav className="flex-1 py-3 overflow-y-auto">
        <div className="px-3 mb-1">
          <span className="text-[10px] font-bold text-tx3 uppercase tracking-widest px-2">Админ цэс</span>
        </div>
        {navItems.map(item => {
          const active = path === item.href || path.startsWith(item.href + '/')
          const count = item.href === '/tasks' ? overdueCount : 0
          return (
...
// Менежер sidebar - Хяналтын эрхтэй
export function ManagerSidebar({
  ...
  const navItems: SidebarItem[] = [
    { label: 'Хяналтын самбар', href: '/', icon: '📊' },
    { label: 'Хурлын жагсаалт', href: '/meetings', icon: '📋' },
    { label: 'Үүрэг даалгавар', href: '/tasks', icon: '✅' },
    { label: 'Биелэлт хянах', href: '/biyelelt', icon: '📈' },
  ]
...
// Ажилтан sidebar - Хязгаарлагдмал эрхтэй
export function EmployeeSidebar({
  ...
  const navItems: SidebarItem[] = [
    { label: 'Миний самбар', href: '/', icon: '📊' },
    { label: 'Миний үүрэг', href: '/tasks', icon: '✅' },
    { label: 'Хурлууд', href: '/meetings', icon: '📋' },
  ...
  return (
    <aside className="w-[220px] flex-shrink-0 bg-surface border-r border-border flex flex-col h-full">
      ...
      <nav className="flex-1 py-3 overflow-y-auto">
        <div className="px-3 mb-1">
          <span className="text-[10px] font-bold text-tx3 uppercase tracking-widest px-2">Миний цэс</span>
        </div>
        {navItems.map(item => {
          const active = path === item.href || path.startsWith(item.href + '/')
          const count = item.href === '/tasks' ? overdueCount : 0
...
// Хуучин Sidebar - харцлага зориулж хадгалсан
export function Sidebar(props: SidebarProps & { navItems?: SidebarItem[], bottomItems?: SidebarItem[] }) {
  const path = usePathname()
  
  const navItems = props.navItems || [
    { label: 'Хяналтын самбар', href: '/dashboard', icon: '📊' },
    { label: 'Хурлын жагсаалт', href: '/meetings', icon: '📋' },
    { label: 'Бүх үүрэг', href: '/tasks', icon: '✅' },
    { label: 'Биелэлт илгээх', href: '/biyelelt', icon: '📤' },
  ]

  const bottomItems = props.bottomItems || [
    { label: 'Тохиргоо', href: '/settings', icon: '⚙️' },
  ]
    ...
    <aside className="w-[220px] flex-shrink-0 bg-surface border-r border-border flex flex-col h-full">
      ...
      <div className="px-4 py-4 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-bold leading-tight">ГШХ Систем</div>
            <div className="text-[10px] text-tx3 leading-tight truncate max-w-[140px]">{props.orgName}</div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 py-3 overflow-y-auto">
        <div className="px-3 mb-1">
          <span className="text-[10px] font-bold text-tx3 uppercase tracking-widest px-2">Үндсэн цэс</span>
        </div>
        {navItems.map(item => {
          const active = path === item.href || path.startsWith(item.href + '/')
          const count = item.href === '/tasks' ? (props.overdueCount || 0) : 0
          return (
            <Link key={item.href} href={item.href}
              className={cn(
                'flex items-center gap-2.5 mx-2 px-3 py-2 rounded text-sm transition-all duration-100 border border-transparent',
